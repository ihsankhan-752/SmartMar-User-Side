...
class _ItemInformationCardState extends State<ItemInformationCard> {
  ...
  getProduct() async {
    DocumentSnapshot snap = await FirebaseFirestore.instance.collection("mycart").doc(widget.pdtIds[widget.index]).get();
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    height: 30,
                    decoration:
                        BoxDecoration(color: AppColors.primaryColor.withOpacity(0.5), borderRadius: BorderRadius.circular(6)),
                    child: Row(
                      children: [
                        quantity < 1
                            ? InkWell(
                                onTap: () async {
                                  await FirebaseFirestore.instance.collection("mycart").doc(widget.pdtIds[widget.index]).delete();
                        ...
                        StreamBuilder<DocumentSnapshot>(
                            stream: FirebaseFirestore.instance.collection("mycart").doc(widget.pdtIds[widget.index]).snapshots(),
                            builder: (context, snapshot) {
                              if (!snapshot.hasData) {
                                return Center(
                                  child: CircularProgressIndicator(),
